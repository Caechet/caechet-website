...
export default function Home() {
  return (
    <>
      ...
      {/* MANIFESTO — MOBILE ONLY (its own section below hero) */}
      <section className="manifesto-mobile">
        <div className="manifesto-mobile-header">
          <span>The Manifesto</span>
          <span>Five Truths · Read Twice</span>
        </div>
        {[
          { tag:'Method', body: <><span>We don&apos;t pitch.<br/>We </span><span className="alt">audit.</span></> },
          { tag:'Principle', body: <>The <span className="alt">second purchase</span> is the whole game.</> },
          { tag:'Diagnosis', body: <>Acquisition fills the bucket. Retention fixes the <span className="alt">hole.</span></> },
          { tag:'Promise', body: <>No <span className="strike">PM tax.</span> No <span className="strike">junior</span> in a queue.</> },
          { tag:'The Filter', body: <>Four new brands a quarter. <span className="alt">That&apos;s it.</span></> },
        ].map((item) => (
          <div key={item.tag} className="manifesto-mobile-item">
            <span className="stamp tag">{item.tag}</span>
            <div className="manifesto-mobile-body">{item.body}</div>
          </div>
        ))}
      </section>
        ...
        <div className="wrap">
          ...
          <div className="offer-grid">
            <div className="offer-items">
              {[
                { title:'Live account audit.', sub:'Deliverability · flows · segmentation · attribution. 60 minutes.' },
                { title:'90-day plan.', sub:'Prioritized roadmap, written, yours to keep.' },
                { title:'Retain or Die · eBook.', sub:'41 pages — calendar, frameworks, full lifecycle map.' },
                { title:'ESP migration, on us.', sub:'Done-for-you flows, lists, segments, deliverability — if MRR over $1K/mo.' },
              ].map((item) => (
